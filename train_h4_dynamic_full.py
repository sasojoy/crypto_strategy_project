# -*- coding: utf-8 -*-
"""
train_h4_dynamic_full.py
1小時(=4x15m) Horizon 的分類模型 + 動態ATR止盈止損回測 + 盤勢濾網 +
「日均≈1」目標式門檻搜尋（雙邊二分 + 微調 + 風險懲罰 + 多空不對稱TP/SL）
輸出：
- h4_dynamic/cls_model_h4.pkl
- h4_dynamic/scaler_h4.joblib
- h4_dynamic/opt_h4_dynamic.json
- h4_dynamic/trades_h4.csv
"""
...
# ===== 設定 =====
OUT_DIR = Path("h4_dynamic"); OUT_DIR.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    # 8) 輸出
    with open(OUT_DIR/"cls_model_h4.pkl","wb") as f: pickle.dump(clf, f)
    import joblib; joblib.dump(scaler, OUT_DIR/"scaler_h4.joblib")

    trades_df = pd.DataFrame(best_trades)
    trades_df.to_csv(OUT_DIR/"trades_h4.csv", index=False, encoding="utf-8-sig")
    ...
    with open(OUT_DIR/"opt_h4_dynamic.json","w",encoding="utf-8") as f:
        json.dump(report, f, ensure_ascii=False, indent=2)

    print("\n=== 完成 ===")
    print("最佳解：", json.dumps(best, ensure_ascii=False))
    print("輸出：", OUT_DIR.resolve())
    print("  - 模型：cls_model_h4.pkl, scaler_h4.joblib")
    print("  - 報告：opt_h4_dynamic.json")
    print("  - 交易：trades_h4.csv")
